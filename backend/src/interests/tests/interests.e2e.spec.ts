...
describe('Interests - E2E', () => {
  ...
  describe('GET /interests', () => {
    it('lists with pagination', async () => {
      ...
      const res = await request(server).get('/interests?page=1&limit=1');
    ...
    it('filters by name (contains)', async () => {
      ...
      const res = await request(server).get('/interests?name=bio');
      expect(res.status).toBe(200);
      expect(Array.isArray(res.body.data)).toBe(true);
  ...
  describe('GET /interests/:id', () => {
    it('returns one by id', async () => {
      ...
      const res = await request(server).get(`/interests/${created!.id}`);
      expect(res.status).toBe(200);
